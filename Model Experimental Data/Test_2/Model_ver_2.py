...
import random
import albumentations as A  # 데이터 증강을 위한 라이브러리 추가
...
# 데이터 증강 함수
def apply_augmentation(image, bboxes, class_ids):
    """
...
# 입력 데이터 처리 함수
def process_data(json_file, frame_file, classes_file, output_image_dir, output_label_dir, apply_aug=False):
    """
    ...
    if apply_aug and "train" in output_image_dir and len(bboxes) > 0:
        ...
        for aug_idx in range(3):
            ...
            # 이미지와 바운딩 박스에 증강 적용
            try:
                aug_image, aug_bboxes, aug_class_ids = apply_augmentation(image, bboxes, class_ids)
...
def split_dataset(json_files, frame_files, classes_file, train_ratio=0.8):
    ...
    for i in tqdm(train_indices, desc="학습 데이터 처리 중"):
        _, _ = process_data(json_files[i], frame_files[i], classes_file,
                         TRAIN_IMAGES_DIR, TRAIN_LABELS_DIR, apply_aug=True)  # 학습 데이터에 증강 적용
    
    for i in tqdm(val_indices, desc="검증 데이터 처리 중"):
        _, _ = process_data(json_files[i], frame_files[i], classes_file,
                         VAL_IMAGES_DIR, VAL_LABELS_DIR, apply_aug=False)  # 검증 데이터는 증강 안 함
