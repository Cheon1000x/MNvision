...
def train_model(yaml_config, config):
    ...
    # 공통 학습 매개변수 - 커스텀 해상도 적용
    train_args = {
        'data': yaml_config,
        'epochs': epochs,
        'batch': batch_size,
        'imgsz': [img_height, img_width],  # [높이, 너비] 순서로 설정
        'patience': 20,
        'verbose': True,
        ...
        'overlap_mask': True,
        'close_mosaic': 10,
        'freeze': 0,           # 모든 레이어 학습 (0: freeze, 1: freeze backbone, 2: freeze head)
        
        # 패딩 및 크기 조정 설정
        'rect': False,          # rectangular training 비활성화 (정확한 패딩을 위해)
        'auto_augment': False,  # 자동 증강 비활성화 (커스텀 해상도 유지)
        'single_cls': False,    # 멀티 클래스 유지
    }
    
    print(f"학습 해상도: {img_width}x{img_height} (패딩 자동 적용)")
