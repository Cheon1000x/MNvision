...
def create_labelme_json(image_path, boxes, class_names, confidence_scores, output_dir, camera_type):
    """
...
def yolo_to_labelme(model_path, image_dir, output_base_dir=None, conf_threshold=0.3, camera_type="cam1"):
    """
    ...
    print(f"총 {len(all_image_files)}개의 이미지 파일을 처리합니다. (카메라 타입: {camera_type})")
    ...
    for image_path in all_image_files:
        ...
        try:
            ...
            # LabelMe JSON 생성
            if boxes:
                json_path = create_labelme_json(image_path, boxes, class_names, confidence_scores, output_dir, camera_type)
...
if __name__ == "__main__":
    ...
    yolo_to_labelme(model_path, image_dir, output_dir, conf_threshold, camera_str)
    print(f"자동 라벨링이 완료되었습니다. ({camera_str} 적용) LabelMe에서 결과를 검토하고 수정하세요.")